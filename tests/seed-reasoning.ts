...
import { IReasoning } from "../src/types/Reasoning";

const graph = loadReasoningGraph();

const prompt = "Escrever testes unitários para todas as funcionalidades do CodeAtlas e reorganizar os arquivos de teste co-localizados";
const thought = `Plano adotado:
...
const toolCall = {
    tool: { name: "test_tool", description: "Test tool", inputSchema: { type: "object", properties: {} } } as any,
    result: "success"
};

addReasoning(graph, {
    reasoning: { prompt, thoughtDescription: thought, thoughtDetails: "plan" as any, solution } as IReasoning,
    toolCall
});
